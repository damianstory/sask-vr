...
export default function PostVRPage() {
  ...
  const isReflectionStep = currentStep === 1
  const isSurveyStep = currentStep === 2
  const canAdvance = checkedItems.length >= 1
  const surveyEmbedUrl = useMemo(() => {
    const url = new URL(data.survey.formUrl)
  ...
  const goNext = () => {
    if (isChecklistStep && !canAdvance) return
  ...
  const renderLeftColumn = () => {
    ...
    return (
      <section className="rounded-[var(--radius-panel)] border border-[color:rgba(217,223,234,0.8)] bg-white/90 p-6 shadow-[var(--shadow-float)] backdrop-blur-[var(--glass-blur)] md:p-6">
        ...
        <div className="mt-6 border-t border-[color:rgba(217,223,234,0.9)] pt-5">
          <h2 className="text-[15px] font-[800] uppercase tracking-[0.14em] text-[var(--myb-primary-blue)]">
            {data.survey.legendHeading}
          </h2>
          <div className="mt-4 flex flex-col gap-2.5">
            {data.survey.legend.map((item) => (
              <div
                key={item.value}
                className="flex items-center gap-3 rounded-[var(--radius-card)] border border-[color:rgba(217,223,234,0.9)] bg-[color:rgba(246,250,255,0.9)] px-3 py-2.5"
              >
                <span className="flex h-8 w-8 flex-shrink-0 items-center justify-center rounded-full bg-[var(--myb-navy)] text-[15px] font-[800] text-white">
                  {item.value}
                </span>
                <span className="text-[15px] font-[300] leading-[1.4] text-[var(--myb-navy)]">
                  {item.label}
                </span>
              </div>
            ))}
  ...
  const renderRightColumn = () => {
    ...
    return (
      <section className="flex min-h-[500px] flex-col overflow-hidden rounded-[var(--radius-panel)] border border-[color:rgba(217,223,234,0.8)] bg-white/90 shadow-[var(--shadow-float)] backdrop-blur-[var(--glass-blur)] md:min-h-0 md:flex-1">
        <iframe
          src={surveyEmbedUrl}
  ...
  return (
    ...
    >
      <ProgressBar current={currentStep + 1} total={TOTAL_STEPS} compact />
      <div className="relative min-h-0 overflow-hidden">
        ...
        >
          ...
          <div className="relative mx-auto flex w-full max-w-[720px] flex-col gap-8 md:min-h-0 md:max-w-[var(--max-content-width)] md:grid md:grid-cols-[minmax(0,24rem)_minmax(0,1fr)] md:gap-6 lg:grid-cols-[minmax(0,28rem)_minmax(0,1fr)] lg:gap-8">
            <div className="flex flex-col gap-3 md:self-start">{renderLeftColumn()}</div>

            <div
              className={cn(
                'flex flex-col gap-4 md:min-h-0',
                isSurveyStep ? 'overflow-hidden' : 'md:overflow-y-auto md:overscroll-contain'
              )}
            >
              {renderRightColumn()}
            </div>
